// /**
//  * assign-obligation.js — CXO Obligation Assignment Screen  v2
//  *
//  * Hierarchy: Chapter (dark navy) → Clause (medium slate) → Obligation rows (light)
//  * Drawer: full overlay panel, slides from right, does NOT compress content
//  * Accordions: all start closed
//  */
...
// /* ── DATA: Chapter → Clauses → Obligations ──────────────────── */
...
//           obligations:[
//             { id:'OB-001', text:'Establish and maintain a Board-approved compliance policy covering all aspects of this circular, reviewed at least annually.', dept:null, assignee:null, dueDate:'', priority:'High', status:'Unassigned' },
//             { id:'OB-002', text:'Distribute the approved policy to all relevant departments and obtain signed acknowledgement from department heads within 30 days.', dept:null, assignee:null, dueDate:'', priority:'Medium', status:'Unassigned' },
//           ]
//         },
//         {
...
//             { id:'OB-004', text:'Define and document the Compliance Officer mandate including scope, authority and escalation rights.', dept:null, assignee:null, dueDate:'', priority:'High', status:'Unassigned' },
//           ]
//         },
//         {
//           clauseId:'C1.3', clauseText:'Compliance Committee shall meet quarterly and submit minutes to the regulator.',
//           obligations:[
//             { id:'OB-005', text:'Conduct Compliance Committee meetings at least quarterly with minutes submitted to the regulator within 15 days of each meeting.', dept:null, assignee:null, dueDate:'', priority:'Medium', status:'Unassigned' },
//           ]
//         },
//       ]
//     },
...
//             { id:'OB-007', text:'Train all frontline staff on updated processes and maintain signed acknowledgement records for audit purposes.', dept:null, assignee:null, dueDate:'', priority:'Medium', status:'Unassigned' },
//           ]
//         },
//         {
//           clauseId:'C2.2', clauseText:'Transaction monitoring systems shall detect and report suspicious activity in real-time.',
//           obligations:[
//             { id:'OB-008', text:'Upgrade transaction monitoring systems to detect and report suspicious activity in real-time with a maximum alert latency of 5 seconds.', dept:null, assignee:null, dueDate:'', priority:'Critical', status:'Unassigned' },
//           ]
//         },
//         {
...
//           obligations:[
//             { id:'OB-010', text:'Submit monthly compliance status reports to the regulator in the prescribed format by the 7th of each calendar month without exception.', dept:null, assignee:null, dueDate:'', priority:'High', status:'Unassigned' },
//             { id:'OB-011', text:'Maintain an internal compliance reporting log with maker-checker controls for all regulatory submissions.', dept:null, assignee:null, dueDate:'', priority:'Medium', status:'Unassigned' },
//           ]
//         },
//         {
...
//           obligations:[
//             { id:'OB-013', text:'Report material breaches or near-misses to the regulator within 24 hours of identification, followed by a full incident report within 7 days.', dept:null, assignee:null, dueDate:'', priority:'Critical', status:'Unassigned' },
//           ]
//         },
//       ]
//     },
...
//           obligations:[
//             { id:'OB-015', text:'Review and recertify the access control matrix for all compliance-sensitive functions on a quarterly basis, signed off by the CISO.', dept:null, assignee:null, dueDate:'', priority:'Medium', status:'Unassigned' },
//           ]
//         },
//       ]
//     },
...
//   return _aoGetData(circId).flatMap(ch => ch.clauses.flatMap(cl => cl.obligations));
// }
...
//     <div class="ao-drawer" id="ao-drawer">
//       <div id="ao-drawer-content"></div>
//     </div>
//   </div>
...
//         <div class="ao-head-sub">Assign regulatory obligations to departments &nbsp;·&nbsp; Chapter → Clause → Obligation</div>
//       </div>
...
//               <span class="ao-csel-id">${activeCirc.id}</span>
...
//               ${circs.map(c => `
//               <div class="ao-csel-item ${c.id === activeCirc.id ? 'active' : ''}" onclick="_aoSwitchCirc('${c.id}')">
//                 <div class="ao-csel-row1">
//                   <span class="ao-csel-item-id">${c.id}</span>
//                   <span class="ao-csel-item-reg">${c.regulator || ''}</span>
//                   <span class="ao-risk-pip ao-risk-${(c.risk||'').toLowerCase()}"></span>
//                 </div>
...
//         </select>
//         <select class="ao-flt-sel" id="ao-filter-priority" onchange="_aoApplyFilters()">
...
//     <!-- CHAPTER LIST -->
//     <div class="ao-chapters" id="ao-chapters">
...
//       <div class="ao-bulk-f">
//         <span class="ao-bulk-lbl">Assign to Dept</span>
...
//       </div>
//       <div class="ao-bulk-f">
//         <span class="ao-bulk-lbl">Priority</span>
//         <select class="ao-bulk-sel" id="ao-bulk-priority">
...
// /* ── RENDER CHAPTER — dark navy header ───────────────────────── */
// function _aoRenderChapter(ch, ci, circId) {
//   const obligs      = ch.clauses.flatMap(cl => cl.obligations);
//   const assignedCnt = obligs.filter(o => o.status !== 'Unassigned').length;
//   return `
// <div class="ao-chapter" id="ao-ch-${ch.chapterId}">
//   <div class="ao-ch-head" onclick="_aoToggleChapter('${ch.chapterId}')">
//     <span class="ao-ch-arr" id="ao-ch-arr-${ch.chapterId}">▶</span>
...
//       <span class="ao-ch-meta">${ch.clauses.length} clauses · ${obligs.length} obligations</span>
//       <span class="ao-ch-prog-pill">${assignedCnt}/${obligs.length} assigned</span>
//     </div>
//   </div>
//   <div class="ao-ch-body" id="ao-ch-body-${ch.chapterId}">
//     ${ch.clauses.map((cl, cli) => _aoRenderClause(cl, cli, circId)).join('')}
//   </div>
// </div>`;
// }

// /* ── RENDER CLAUSE — medium slate header ─────────────────────── */
// function _aoRenderClause(cl, cli, circId) {
//   const assignedCnt = cl.obligations.filter(o => o.status !== 'Unassigned').length;
//   const done        = assignedCnt === cl.obligations.length;
//   return `
// <div class="ao-clause" id="ao-cl-${cl.clauseId}">
//   <div class="ao-cl-head" onclick="_aoToggleClause('${cl.clauseId}')">
//     <span class="ao-cl-arr" id="ao-cl-arr-${cl.clauseId}">▶</span>
...
//     </div>
//   </div>
//   <div class="ao-cl-body" id="ao-cl-body-${cl.clauseId}">
//     <!-- Row header -->
//     <div class="ao-ob-header-row">
//       <span></span><span></span>
//       <span class="ao-ob-col-lbl">Obligation</span>
//       <span class="ao-ob-col-lbl">Department</span>
//       <span class="ao-ob-col-lbl">Due Date</span>
//       <span class="ao-ob-col-lbl">Priority</span>
//       <span class="ao-ob-col-lbl">Status</span>
//       <span></span>
//     </div>
//     ${cl.obligations.map(ob => _aoRenderObRow(ob, circId)).join('')}
//   </div>
// </div>`;
// }

// /* ── RENDER OBLIGATION ROW — light ───────────────────────────── */
// function _aoRenderObRow(ob, circId) {
...
//   return `
// <div class="ao-ob-row ${ob.status === 'Unassigned' ? 'ao-ob-unassigned' : ''}"
//      id="ao-row-${ob.id}"
//      data-obid="${ob.id}" data-circid="${circId}"
//      data-status="${ob.status}" data-priority="${ob.priority}" data-dept="${ob.dept||''}">

//   <label class="ao-check-wrap ao-ob-chk" onclick="event.stopPropagation()">
//     <input type="checkbox" class="ao-row-chk" data-id="${ob.id}" onchange="_aoRowCheck(this)"/>
//     <span class="ao-checkmark"></span>
//   </label>

//   <span class="ao-ob-id">${ob.id}</span>

//   <div class="ao-ob-text-cell" onclick="_aoOpenDrawer('${ob.id}','${circId}')">
//     <div class="ao-ob-text">${ob.text}</div>
//   </div>

//   <div onclick="event.stopPropagation()">
//     <select class="ao-inline-sel ${ob.dept ? 'filled' : ''}"
//             onchange="_aoInlineDeptChange('${ob.id}','${circId}',this)">
...
//   <span class="ao-st-badge ${stCls}">${ob.status}</span>

//   <button class="ao-ob-open-btn" onclick="_aoOpenDrawer('${ob.id}','${circId}')" title="Open detail">›</button>
// </div>`;
// }

// /* ── OVERLAY DRAWER ──────────────────────────────────────────── */
// window._aoOverlayClick = function(e) {
//   if (e.target.id === 'ao-overlay') _aoCloseDrawerDirect();
// };

// window._aoOpenDrawer = function(obId, circId) {
//   const chapters = _aoGetData(circId);
//   let ob = null, clauseRef = '', clauseText = '', chapterTitle = '';
//   for (const ch of chapters) {
//     for (const cl of ch.clauses) {
//       const f = cl.obligations.find(o => o.id === obId);
//       if (f) { ob = f; clauseRef = cl.clauseId; clauseText = cl.clauseText; chapterTitle = ch.chapterTitle + ' — ' + ch.chapterSub; break; }
//     }
//     if (ob) break;
//   }
//   if (!ob) return;
...
//       <span class="ao-dr-ob-id">${ob.id}</span>
//       <span class="ao-pri-badge ${priCls}">${ob.priority}</span>
//     </div>
//     <button class="ao-dr-close" onclick="_aoCloseDrawerDirect()">✕</button>
//   </div>

//   <!-- Chapter + Clause context (visual hierarchy in drawer too) -->
//   <div class="ao-dr-ctx">
//     <div class="ao-dr-ctx-chapter">${chapterTitle}</div>
//     <div class="ao-dr-ctx-clause-row">
//       <span class="ao-dr-ctx-clause-id">${clauseRef}</span>
//       <span class="ao-dr-ctx-clause-text">${clauseText}</span>
//     </div>
//   </div>
...
//     <div class="ao-dr-field-row">
//       <div class="ao-dr-field">
//         <label class="ao-dr-label">Due Date</label>
//         <input type="date" class="ao-dr-input" id="ao-dr-due-${obId}" value="${ob.dueDate || ''}"/>
//       </div>
//       <div class="ao-dr-field">
//         <label class="ao-dr-label">Priority</label>
//         <select class="ao-dr-input" id="ao-dr-pri-${obId}">
//           ${['Critical','High','Medium','Low'].map(p => `<option ${p === ob.priority ? 'selected' : ''}>${p}</option>`).join('')}
//         </select>
//       </div>
//     </div>
...
//                 placeholder="Add context or instructions for this obligation…">${ob._notes || ''}</textarea>
//     </div>
...
//   const inp = document.getElementById(`ao-dr-assignee-${obId}`);
...
//   const chapters = _aoGetData(circId);
//   let ob = null;
//   for (const ch of chapters) { for (const cl of ch.clauses) { const f = cl.obligations.find(o => o.id === obId); if (f) { ob = f; break; } } if (ob) break; }
//   if (!ob) return;
...
//   ob.priority = document.getElementById(`ao-dr-pri-${obId}`)?.value      || ob.priority;
...
//   if (row) row.outerHTML = _aoRenderObRow(ob, circId);
...
//   const chapters = _aoGetData(circId);
//   for (const ch of chapters) { for (const cl of ch.clauses) { const f = cl.obligations.find(o => o.id === obId); if (f) { f.dueDate = inp.value; inp.classList.toggle('filled', !!inp.value); return; } } }
// };
...
//   document.querySelectorAll('.ao-row-chk').forEach(c => c.checked = false);
...
//   const pri  = document.getElementById('ao-bulk-priority')?.value;
...
// /* ── ACCORDION TOGGLES ───────────────────────────────────────── */
// window._aoToggleChapter = function(chId) {
//   const body = document.getElementById(`ao-ch-body-${chId}`);
//   const arr  = document.getElementById(`ao-ch-arr-${chId}`);
//   if (!body) return;
//   const open = body.classList.contains('open');
//   body.classList.toggle('open', !open);
//   if (arr) arr.textContent = open ? '▶' : '▼';
// };
// window._aoToggleClause = function(clId) {
//   const body = document.getElementById(`ao-cl-body-${clId}`);
//   const arr  = document.getElementById(`ao-cl-arr-${clId}`);
//   if (!body) return;
//   const open = body.classList.contains('open');
//   body.classList.toggle('open', !open);
//   if (arr) arr.textContent = open ? '▶' : '▼';
...
//   const fP = document.getElementById('ao-filter-priority')?.value || '';
//   const fD = document.getElementById('ao-filter-dept')?.value     || '';
//   document.querySelectorAll('.ao-ob-row').forEach(row => {
//     const ok = (!fS || row.dataset.status === fS) && (!fP || row.dataset.priority === fP) && (!fD || row.dataset.dept === fD);
...
// window._aoSwitchCirc = function(circId) {
//   document.getElementById('ao-csel-drop').style.display = 'none';
//   renderAssignObligation(circId);
// };
// window._aoSaveAll = function() {
...
//     document.querySelectorAll('.ao-csel-item').forEach(i => i.style.display = i.textContent.toLowerCase().includes(q) ? '' : 'none');
//   });
//   document.addEventListener('click', e => {
//     const wrap = document.getElementById('ao-csel-wrap');
...
//   --ao-rl:       12px;
//   --ao-rxl:      16px;
//   --ao-sh:       0 1px 4px rgba(30,36,51,.07);
//   --ao-shm:      0 4px 16px rgba(30,36,51,.12);
//   --ao-shl:      0 8px 32px rgba(30,36,51,.18);
// }
...
// .ao-wrap{max-width:1100px;margin:0 auto;padding:28px 24px 100px;}
...
// .ao-btn-pri{background:var(--ao-text);color:#fff;} .ao-btn-pri:hover{background:#2d3548;box-shadow:0 4px 12px rgba(30,36,51,.2);}
...
// .ao-head-title{font-size:24px;font-weight:800;color:var(--ao-text);margin-bottom:4px;}
// .ao-head-sub{font-size:12px;color:var(--ao-text-mut);}
// .ao-head-right{display:flex;gap:8px;}

// /* CIRC CARD */
// .ao-circ-card{display:flex;align-items:center;gap:20px;background:var(--ao-card);border:1px solid var(--ao-border);border-radius:var(--ao-rxl);padding:18px 22px;margin-bottom:14px;box-shadow:var(--ao-sh);flex-wrap:wrap;}
...
// .ao-csel-id{font-family:'DM Mono',monospace;font-size:11px;font-weight:700;color:var(--ao-accent);background:var(--ao-accent-lt);border:1px solid var(--ao-accent-md);padding:2px 8px;border-radius:4px;flex-shrink:0;}
...
// .ao-csel-search{width:100%;padding:10px 14px;background:var(--ao-bg);border:none;border-bottom:1px solid var(--ao-border);font-family:inherit;font-size:12px;color:var(--ao-text);outline:none;box-sizing:border-box;}
...
// .ao-csel-item-id{font-family:'DM Mono',monospace;font-size:11px;font-weight:700;color:var(--ao-text);}
...
// .ao-stat-num{font-size:26px;font-weight:800;color:var(--ao-text);line-height:1;}
...
// /* TOOLBAR */
// .ao-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 16px;background:var(--ao-card);border:1px solid var(--ao-border);border-radius:var(--ao-r);margin-bottom:14px;box-shadow:var(--ao-sh);}
...
// /* CHAPTERS */
// .ao-chapters{display:flex;flex-direction:column;gap:10px;}

// /* ── CHAPTER — DARK NAVY ── */
// .ao-chapter{border-radius:var(--ao-rl);overflow:hidden;box-shadow:var(--ao-shm);}
// .ao-ch-head{display:flex;align-items:center;gap:14px;padding:14px 20px;background:var(--ao-cl-bg);cursor:pointer;user-select:none;transition:background .15s;}
// .ao-ch-head:hover{background:#202840;}
// .ao-ch-arr{font-size:9px;color:rgba(255,255,255,.35);flex-shrink:0;width:12px;transition:color .13s;}
// .ao-ch-head:hover .ao-ch-arr{color:rgba(255,255,255,.75);}
// .ao-ch-titles{display:flex;flex-direction:column;gap:2px;flex:1;min-width:0;}
// .ao-ch-num{font-size:13px;font-weight:800;color:var(--ao-ch-text);letter-spacing:.01em;}
// .ao-ch-sub{font-size:11px;color:var(--ao-ch-sub);}
// .ao-ch-right{display:flex;align-items:center;gap:10px;flex-shrink:0;}
// .ao-ch-meta{font-size:11px;color:rgba(255,255,255,.4);}
...
// .ao-ch-body{display:none;}
// .ao-ch-body.open{display:block;}

// /* ── CLAUSE — MEDIUM SLATE ── */
// .ao-clause{}
// .ao-cl-head{display:flex;align-items:center;gap:12px;padding:10px 20px 10px 30px;background:#5a6578;cursor:pointer;user-select:none;border-top:1px solid var(--ao-cl-bdr);transition:background .15s;}
// .ao-cl-head:hover{background:#364972;}
// .ao-cl-arr{font-size:8px;color:rgba(220,230,245,.3);flex-shrink:0;width:10px;}
// .ao-cl-head:hover .ao-cl-arr{color:rgba(220,230,245,.75);}
// .ao-cl-id{font-family:'DM Mono',monospace;font-size:11px;font-weight:700;color:var(--ao-cl-id);background:rgba(126,200,227,.12);border:1px solid rgba(126,200,227,.22);padding:2px 8px;border-radius:4px;flex-shrink:0;}
// .ao-cl-text{flex:1;font-size:12px;color:var(--ao-cl-text);line-height:1.45;}
// .ao-cl-right{display:flex;align-items:center;gap:8px;flex-shrink:0;}
// .ao-cl-ob-cnt{font-size:10px;color:rgba(220,230,245,.45);}
// .ao-cl-prog{font-size:10px;font-weight:700;padding:2px 9px;border-radius:20px;background:rgba(255,255,255,.08);color:rgba(220,230,245,.65);border:1px solid rgba(255,255,255,.1);}
// .ao-cl-prog.done{background:rgba(14,159,110,.22);color:#6ee7b7;border-color:rgba(14,159,110,.3);}
// .ao-cl-body{display:none;background:var(--ao-card);}
// .ao-cl-body.open{display:block;}

// /* Column header row inside clause */
// .ao-ob-header-row{display:grid;grid-template-columns:36px 72px 1fr 150px 130px 88px 100px 36px;padding:6px 16px 6px 28px;background:#f8f9fb;border-bottom:1px solid var(--ao-border-lt);}
// .ao-ob-col-lbl{font-size:9px;font-weight:700;color:var(--ao-text-mut);text-transform:uppercase;letter-spacing:.07em;}

// /* ── OBLIGATION ROW — LIGHT ── */
// .ao-ob-row{display:grid;grid-template-columns:36px 72px 1fr 150px 130px 88px 100px 36px;align-items:center;padding:10px 16px 10px 28px;border-bottom:1px solid var(--ao-border-lt);background:var(--ao-card);transition:background .12s;animation:aoIn .15s ease;}
// .ao-ob-row:last-child{border-bottom:none;}
...
// .ao-ob-chk{justify-self:center;}
// .ao-ob-id{font-family:'DM Mono',monospace;font-size:10px;font-weight:700;color:var(--ao-purple);background:var(--ao-purple-lt);border:1px solid #d4d6f8;padding:2px 7px;border-radius:4px;white-space:nowrap;}
// .ao-ob-text-cell{cursor:pointer;padding:0 10px;}
...
// .ao-ob-text-cell:hover .ao-ob-text{color:var(--ao-accent);}
...
// .ao-st-badge{font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px;white-space:nowrap;text-align:center;}
...
// .ao-dr-inner{padding:26px 24px;display:flex;flex-direction:column;gap:18px;min-height:100%;}
...
// .ao-dr-ob-id{font-family:'DM Mono',monospace;font-size:12px;font-weight:700;color:var(--ao-purple);background:var(--ao-purple-lt);border:1px solid #d4d6f8;padding:3px 10px;border-radius:5px;}
...
// /* Context block — shows chapter + clause hierarchy */
// .ao-dr-ctx{background:var(--ao-cl-bg);border-radius:var(--ao-r);padding:14px 16px;display:flex;flex-direction:column;gap:8px;}
// .ao-dr-ctx-chapter{font-size:10px;font-weight:700;color:rgba(255,255,255,.45);text-transform:uppercase;letter-spacing:.08em;}
// .ao-dr-ctx-clause-row{display:flex;align-items:flex-start;gap:9px;}
// .ao-dr-ctx-clause-id{font-family:'DM Mono',monospace;font-size:11px;font-weight:700;color:var(--ao-cl-id);background:rgba(126,200,227,.15);border:1px solid rgba(126,200,227,.25);padding:2px 8px;border-radius:4px;flex-shrink:0;}
// .ao-dr-ctx-clause-text{font-size:12px;color:var(--ao-cl-text);line-height:1.5;}
...
// /* Fields */
// .ao-dr-section-label{font-size:10px;font-weight:700;color:var(--ao-text-mut);text-transform:uppercase;letter-spacing:.08em;padding-top:2px;border-top:1px solid var(--ao-border-lt);}
// .ao-dr-fields{display:flex;flex-direction:column;gap:14px;}
// .ao-dr-field{display:flex;flex-direction:column;gap:5px;}
// .ao-dr-field-row{display:grid;grid-template-columns:1fr 1fr;gap:12px;}
...
// .ao-dr-ta{min-height:82px;resize:vertical;}
...
// .ao-sug-name{flex:1;font-size:13px;color:var(--ao-text);font-weight:500;}
...
// /* Drawer footer */
// .ao-dr-foot{display:flex;gap:8px;justify-content:flex-end;padding-top:14px;border-top:1px solid var(--ao-border-lt);margin-top:auto;}
...
//   background:var(--ao-ch-bg);
//   border-radius:14px;
//   box-shadow:0 8px 40px rgba(10,15,28,.35);
//   z-index:200;overflow:hidden;
//   animation:aoIn .22s ease;
//   white-space:nowrap;
// }
...
// .ao-bulk-clear{background:none;border:1px solid rgba(255,255,255,.18);border-radius:6px;color:rgba(255,255,255,.55);font-family:inherit;font-size:11px;padding:3px 10px;cursor:pointer;transition:all .13s;}
...
// .ao-bulk-lbl{font-size:9px;font-weight:700;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.07em;}
// .ao-bulk-sel{padding:7px 10px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.15);border-radius:6px;font-family:inherit;font-size:12px;color:#fff;outline:none;cursor:pointer;}
// .ao-bulk-inp{padding:7px 10px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.15);border-radius:6px;font-family:inherit;font-size:12px;color:#fff;outline:none;color-scheme:dark;}
